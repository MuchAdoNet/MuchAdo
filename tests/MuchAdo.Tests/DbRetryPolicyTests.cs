...
internal sealed class DbRetryPolicyTests
{
	...
	{
		var settings = new DbConnectorSettings { ConnectionRetryPolicy = new FakeDbRetryPolicy() };
	...
	{
		var settings = new DbConnectorSettings { ConnectionRetryPolicy = PollyDbRetryPolicy.Create(ResiliencePipeline.Empty) };
		using var connector = new DbConnector(new SqliteConnection("Data Source=:memory:"), settings);
